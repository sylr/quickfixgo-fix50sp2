...
// SetCommRate sets CommRate, Tag 1233.
func (m NoBidComponents) SetCommRate(value decimal.Decimal, scale int32) {
	m.Set(field.NewCommRate(value, scale))
}

// SetCommUnitOfMeasure sets CommUnitOfMeasure, Tag 1238.
func (m NoBidComponents) SetCommUnitOfMeasure(v string) {
	m.Set(field.NewCommUnitOfMeasure(v))
}
...
// GetCommRate gets CommRate, Tag 1233.
func (m NoBidComponents) GetCommRate() (v decimal.Decimal, err quickfix.MessageRejectError) {
	var f field.CommRateField
	if err = m.Get(&f); err == nil {
		v = f.Value()
	}
	return
...
// GetCommUnitOfMeasure gets CommUnitOfMeasure, Tag 1238.
func (m NoBidComponents) GetCommUnitOfMeasure() (v string, err quickfix.MessageRejectError) {
	var f field.CommUnitOfMeasureField
	if err = m.Get(&f); err == nil {
		v = f.Value()
	}
	return
...
// HasCommRate returns true if CommRate is present, Tag 1233.
func (m NoBidComponents) HasCommRate() bool {
	return m.Has(tag.CommRate)
}

// HasCommUnitOfMeasure returns true if CommUnitOfMeasure is present, Tag 1238.
func (m NoBidComponents) HasCommUnitOfMeasure() bool {
	return m.Has(tag.CommUnitOfMeasure)
}
	...
	return NoBidComponentsRepeatingGroup{
		quickfix.NewRepeatingGroup(
			...
			quickfix.GroupTemplate{
				...
				quickfix.GroupElement(tag.CommRate),
				quickfix.GroupElement(tag.CommUnitOfMeasure),
